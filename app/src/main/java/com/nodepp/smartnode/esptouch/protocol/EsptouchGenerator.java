package com.nodepp.smartnode.esptouch.protocol;


import com.nodepp.smartnode.esptouch.task.IEsptouchGenerator;
...
public class EsptouchGenerator implements IEsptouchGenerator {
	...
	public EsptouchGenerator(String apSsid, String apBssid, String apPassword,
		...
		char[] gcU81 = gc.getU8s();		//guidesU8s[0] = 515;	guidesU8s[1] = 514;guidesU8s[2] = 513;guidesU8s[3] = 512;
		mGcBytes2 = new byte[gcU81.length][];

		for (int i = 0; i < mGcBytes2.length; i++) {
			mGcBytes2[i] = ByteUtil.genSpecBytes(gcU81[i]);//mGcBytes2[0]赋值是515个1，其他同理
		}

...
/**
 * Created by yuyue on 2016/11/26.
 */
public class ProgressButton extends TextView {
    ...
    //背景颜色
    private int mBackgroundColor = 0xFF3FA7EE;
    //下载中后半部分后面背景颜色
    private int mBackgroundSecondColor = Color.LTGRAY;
    //文字颜色
    ...
    private float mButtonRadius = 20;
    ...
    private void init() {
        ...
        mDot1Paint.setTextSize(Dp2Px(mContext,10));

        //设置第二个点画笔
        mDot2Paint = new Paint();
        mDot2Paint.setAntiAlias(true);
        mDot2Paint.setTextSize(Dp2Px(mContext,10));
    ...
    private void drawBackground(Canvas canvas) {
        ...
        mButtonRadius = Dp2Px(mContext,30);
    ...
    private void drawTextAbove(Canvas canvas) {
        ...
        switch (mState) {
            ...
            case DOWNLOADING:
                ...
                canvas.drawText(mCurrentText.toString(), (getMeasuredWidth() - textWidth) / 2, y, mTextPaint);
                canvas.drawCircle((getMeasuredWidth() + textWidth) / 2 + mDot1transX, y, 4, mDot1Paint);
                canvas.drawCircle((getMeasuredWidth() + textWidth) / 2 + 20 + mDot2transX, y, 4, mDot2Paint);
                canvas.drawCircle((getMeasuredWidth() + textWidth) / 2 + 40 + mDot2transX, y, 4, mDot2Paint);
                break;
            case INSTALLING:
                mTextPaint.setColor(mTextCoverColor);
                canvas.drawText(mCurrentText.toString(), (getMeasuredWidth() - textWidth) / 2, y, mTextPaint);
    ...
    public void setState(int state) {
        if (mState != state) {//状态确实有改变
            this.mState = state;
            invalidate();
            if (state == ProgressButton.DOWNLOADING) {
                //开启两个点动画
